...
/**
 * Description: 组合效果，先对目标 Path 使用一个 PathEffect，然后再对这个改变后的 Path 使用另一个 PathEffect
 * @author tangzhentao
 * @since 2020/5/6
 */
class ComposePathEffectView: View{
    ...
    override fun onDraw(canvas: Canvas?) {
        ...
        paint.pathEffect = ComposePathEffect(dash, discrete)
        val path = Path()
        path.reset()
